...
This interaction is done over a \emph{Rest} interface.

\subsection{Structure}
The server is composed of several files:

\begin{description}
  \item[Server file] This is the main file that holds the code to execute the server. When we want to start the server, it must be with \emph{sudo} because of the SSL integration.
  \item[Custom adapter file] This is the file that overrides the SSL class so that the client is verified using its certificates.
  \item[Cipher file] This file holds auxiliary cryptographic functions that are used by the server.
  \item[Checker file] Holds auxiliary functions that make certain verifications before each request to the \emph{HTTP Rest API}, such as making sure the user is logged in before the player tries to download a file.
\end{description}
...
\begin{figure}[H]
\centerline{\includegraphics[width=500pt]{images/webpage.png}}
